import { AssetItemData } from '@/components/AssetItem';

export const deleteItem = (id: number) => {
  const index = MOCK_ASSETS.findIndex(item => item.id === id);
  if (index !== -1) {
    MOCK_ASSETS.splice(index, 1);
  }
}

export const setItemFavorite = (id: number, favorite: boolean) => {
  const index = MOCK_ASSETS.findIndex(item => item.id === id);
  if (index !== -1) {
    MOCK_ASSETS[index].favorite = favorite;
  }
}

export const MOCK_ASSETS: AssetItemData[] = [
  {
    id: 1,
    name: '唯卓仕 85mm F1.8',
    price: 1853.89,
